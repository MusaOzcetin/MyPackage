...
# Select the best genome (max fitness) from each generation
function select_best_genomes(species_genomes_per_gen)
    ...
    for (i, genomes) in enumerate(species_genomes_per_gen)
        ...
        max_idx = findmax(fitnesses)[2]  # findmax returns (max_val, index)
        best_genomes[i] = genomes[max_idx]
    end
    return best_genomes
end

# Plot average genome complexity: avg nodes and connections per generation
...
# Boxplot fitness distribution per generation
function plot_fitness_boxplot(fitness_data::Vector{Vector{Float64}})
    ...
    boxplot(groups, values;
        ...
        color = :lightblue,
        legend = false,
        xticks = (1:maximum(groups), string.(1:maximum(groups))))
end

# Visualize a single genome’s neural network
function plot_genome_network(genome)
    ...
    node_types = [nodes[id].nodetype for id in node_ids]
    ...
    for t in [:input, :hidden, :output]
        ids_t = filter(id -> nodes[id].nodetype == t, node_ids)
    ...
    graphplot(src, dst;
        ...
        nodestrokec = :black,
        nodesize = 0.15,
        # edgelabel = round.(weights, digits=2), # avoid reshape issues
...
# Animate the best genome over generations within a species
